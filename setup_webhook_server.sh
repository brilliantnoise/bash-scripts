...
function runDeploy(dir, branch, pm2Name, cb) {
  ...
  // install/build use shell conditionals checked AFTER git reset so pnpm<->npm switches
  // are always detected from the actual current lockfile, not the pre-deploy stale state.
  ...
  const cmds = [
    // Fix ownership before git so gitdeploy can unlink APP_USER-owned files,
    // and after so APP_USER can write/chmod files git just created (both npm and pnpm need this).
    `sudo ${FIX_PERMS_HELPER} '${safeDir}'`,
